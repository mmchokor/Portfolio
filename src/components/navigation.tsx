...
export function Navigation() {
  ...
  const scrollAfterHomeNavigationRef = useRef(false);
  ...
  const handleBrandClick = (event: MouseEvent<HTMLAnchorElement>) => {
    ...
    scrollAfterHomeNavigationRef.current = true;
  };

  useEffect(() => {
    if (pathname !== "/" || !scrollAfterHomeNavigationRef.current) return;
    scrollAfterHomeNavigationRef.current = false;
  ...
  return <header className="site-header"><nav className="nav-shell" aria-label="Primary navigation">
    <Link href="/" className="brand" aria-label="Mohamed Chokor, home" onClick={handleBrandClick}><span>Chokor</span><span className="brand-accent">.Portfolio()</span></Link>
